...
if not os.path.exists(os.path.join('exp-11-13', save_txt)):
    os.makedirs(os.path.join('exp-11-13', save_txt))
for i in range(318):
    with open(os.path.join(label_path, file_name + '_' + str(i) + ".txt"), 'r') as f:
        content = f.readlines()
        detection = content2detections(content, ax)
    with open(os.path.join('exp-11-13', save_txt) + f'/{i}.txt', 'w') as file:
