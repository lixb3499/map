...
label_path = "example_5_校正角度/saved_txt"
file_name = ''
save_txt = 'save_txt'
...
with open(os.path.join(label_path, file_name + str(0) + ".txt"), 'r') as f:
    content = f.readlines()
    tracker = Map(content, [area1, area2])
...
while (True):
    ...
    for area in tracker.areas:
        cv2.putText(frame, f"count of area_{area.id}:    {area.count_car}", (25, 100+25*area.id), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255),
                    2)
