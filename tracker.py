...
class Tracks:
    ...
    def draw(self, img):
        ...
        if self.ifstop(self.v_Threshold):
            if self.id_matched:
                cv2.putText(img, f"Tracking  ID={self.track_id}, stop, t = {self.stoptime:.2f}s, licence={self.licence}",
            ...
            if self.id_matched:
                cv2.putText(img, f"Tracking  ID={self.track_id}, V={3.6 * self.frame_rate * self.v_average:.2f}km/h, licence={self.licence}",
                            (int(self.target_box[0] - 30), int(self.target_box[1] - 5)), cv2.FONT_HERSHEY_SIMPLEX,
                            0.7, (255, 0, 0), 2)
            else:
                cv2.putText(img,
                            f"Lost ID={self.track_id}, V={3.6 * self.frame_rate * vector_norm(self.trace_v_list[-1]):.2f}km/h, licence={self.licence}",
                            (int(self.target_box[0] - 30), int(self.target_box[1] - 5)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7, (255, 255, 0), 2)
